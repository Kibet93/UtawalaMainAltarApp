  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await ReservedSeatsService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await ReservedSeatsService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.reservedSeats.create.success'),
        );
        routerAsync().push('/reserved-seats');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await ReservedSeatsService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.reservedSeats.update.success'),
        );
        routerAsync().push('/reserved-seats');
      } catch (error) {
        Errors.handle(error);
