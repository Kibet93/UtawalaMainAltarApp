...
import reservedSeatsListExporterFields from '@/modules/reserved-seats/reserved-seats-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !reservedSeatsListExporterFields ||
          !reservedSeatsListExporterFields.length
        ) {
          throw new Error(
            'reservedSeatsListExporterFields is required',
          );
        }
        ...
        const response = await ReservedSeatsService.list(
          filter,
          getters.orderBy,
          null,
          null,
