...
import categoryListExporterFields from '@/modules/category/category-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !categoryListExporterFields ||
          !categoryListExporterFields.length
        ) {
          throw new Error(
            'categoryListExporterFields is required',
          );
        }
