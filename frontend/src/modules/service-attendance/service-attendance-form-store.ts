  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await ServiceAttendanceService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await ServiceAttendanceService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.serviceAttendance.create.success'),
        );
        routerAsync().push('/service-attendance');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await ServiceAttendanceService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.serviceAttendance.update.success'),
        );
        routerAsync().push('/service-attendance');
      } catch (error) {
        Errors.handle(error);
