  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await AnnouncementTargetsService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await AnnouncementTargetsService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.announcementTargets.create.success'),
        );
        routerAsync().push('/announcement-targets');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await AnnouncementTargetsService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.announcementTargets.update.success'),
        );
        routerAsync().push('/announcement-targets');
      } catch (error) {
        Errors.handle(error);
