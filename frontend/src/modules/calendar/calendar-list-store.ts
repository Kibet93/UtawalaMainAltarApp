...
import calendarListExporterFields from '@/modules/calendar/calendar-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !calendarListExporterFields ||
          !calendarListExporterFields.length
        ) {
          throw new Error(
            'calendarListExporterFields is required',
          );
        }
