  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await HomeFellowshipsService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await HomeFellowshipsService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.homeFellowships.create.success'),
        );
        routerAsync().push('/home-fellowships');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await HomeFellowshipsService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.homeFellowships.update.success'),
        );
        routerAsync().push('/home-fellowships');
      } catch (error) {
        Errors.handle(error);
