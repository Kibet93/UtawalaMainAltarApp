...
import homeFellowshipsListExporterFields from '@/modules/home-fellowships/home-fellowships-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !homeFellowshipsListExporterFields ||
          !homeFellowshipsListExporterFields.length
        ) {
          throw new Error(
            'homeFellowshipsListExporterFields is required',
          );
        }
        ...
        const response = await HomeFellowshipsService.list(
          filter,
          getters.orderBy,
          null,
          null,
