...
import liveServiceListExporterFields from '@/modules/live-service/live-service-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !liveServiceListExporterFields ||
          !liveServiceListExporterFields.length
        ) {
          throw new Error(
            'liveServiceListExporterFields is required',
          );
        }
        ...
        const response = await LiveServiceService.list(
          filter,
          getters.orderBy,
          null,
          null,
