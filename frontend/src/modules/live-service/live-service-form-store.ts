  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await LiveServiceService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await LiveServiceService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.liveService.create.success'),
        );
        routerAsync().push('/live-service');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await LiveServiceService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.liveService.update.success'),
        );
        routerAsync().push('/live-service');
      } catch (error) {
        Errors.handle(error);
