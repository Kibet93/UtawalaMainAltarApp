...
import membersListExporterFields from '@/modules/members/members-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !membersListExporterFields ||
          !membersListExporterFields.length
        ) {
          throw new Error(
            'membersListExporterFields is required',
          );
        }
