...
import announcementListExporterFields from '@/modules/announcement/announcement-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !announcementListExporterFields ||
          !announcementListExporterFields.length
        ) {
          throw new Error(
            'announcementListExporterFields is required',
          );
        }
